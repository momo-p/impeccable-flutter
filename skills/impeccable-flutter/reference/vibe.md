...
Showing beats asking. A theme is a small number of decisions (palette, type, shape, density, motion). A person who cannot answer "what vibe do you want?" in the abstract will answer it in half a second when looking at three options.
...
Name each one. "Field notes", "Broadcast", "Quiet utility". Then give it a single line of point of view. A direction you cannot describe in a sentence is not a direction.
...
- **The palette as role swatches** with hex values: primary, on-primary, surface, surface-container, on-surface, on-surface-variant, outline, error. Roles, not a rainbow: this is what gets copied into the theme.
...
They will often take one and pull something from another. That is a good outcome rather than indecision. Record the result, not the original.
